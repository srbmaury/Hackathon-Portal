...
describe('hackathons API (AI suggestions)', () => {
  ...
  it('assignHackathonRole calls API.post with correct params', async () => {
    const hackathonId = 'hackathon-1';
    const userId = 'user-1';
    const role = 'mentor';

    API.post.mockResolvedValue({ data: 'assign-role-data' });

    const data = await hackathonsApi.assignHackathonRole(
      hackathonId,
      userId,
      role,
      token
    ...
    expect(data).toBe('assign-role-data');
  });
});
